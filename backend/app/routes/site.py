...
from app.models.models import SiteAddedSource, Site, User
...
def get_scanned_urls(site_id):
    ...
    sources_query = SiteAddedSource.query.filter_by(site_id=site_id).all()
    sources = [source.to_dict() for source in sources_query]
    ...
    query_results = chroma_client.get_collection(collection_name).get(where={"source": {"$in": all_sources}}, include=["metadatas"]) 
    ...
    for source in sources:
        source["is_used"] = source["source"] in used_sources
...
def delete_added_source(site_id, added_source_id):
    ...
    added_source = SiteAddedSource.query.filter_by(id=added_source_id, site_id=site_id).first()
    ...
    collection_name = f"{site.name}_{user_id}"
    query_results = chroma_client.get_collection(collection_name).get(where={"source": added_source.source}, include=["metadatas"]) 
